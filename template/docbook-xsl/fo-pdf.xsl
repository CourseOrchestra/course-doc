...
<xsl:stylesheet version="1.0"
	...
	<!--  -->
	<xsl:param name="met.table.head.font.size">0.8</xsl:param>
  ...
  <xsl:param name="appendix.autolabel">A</xsl:param>
	...
	<xsl:template match="title | db:title" mode="chapter.titlepage.recto.auto.mode">
		<fo:block xsl:use-attribute-sets="chapter.titlepage.recto.style"  margin-bottom="1cm" margin-top="5cm">
  ...
  <xsl:template match="formalpara/title">
	...
	<xsl:param name="header.column.widths">2 0 17</xsl:param>
			...
			<xsl:choose>
				...
				<xsl:when test="$position='left'">
					<!-- Same for odd, even, empty, and blank sequences -->
						<fo:external-graphic content-height="0.4cm" src="url(logo.png)">
						  </fo:external-graphic>
				</xsl:when>

				<xsl:when test="($sequence='odd' or $sequence='even') and $position='right' and $pageclass != 'lot'">
